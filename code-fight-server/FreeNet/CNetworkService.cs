...
namespace FreeNet
{
    public class CNetworkService
    {
		...
		void on_new_client(Socket client_socket, object token)
		{
            ...
            StringBuilder str_tmp = new StringBuilder(string.Format("\r\n[{0}] A client connected. handle {1},  count {2}, ip {3}, time {4}",
                Thread.CurrentThread.ManagedThreadId, client_socket.Handle,
                this.connected_count,
                client_socket.RemoteEndPoint.ToString(), DateTime.Now.ToString()  + "\r\n"));

            Console.Write(str_tmp.ToString());
            save_log(str_tmp.ToString());
                // 플에서 하나 꺼내와 사용한다.
        ...
        public void save_log(string str_tmp)
        {
            lock(this)
            {
                File.AppendAllText("./log/connected.txt", str_tmp);
            }
        }
